...
api.send = async function({template, message, locals}) {
  ...
  if(bedrock.config.mail.log.headers ||
    bedrock.config.mail.log.text ||
    bedrock.config.mail.log.html) {
    let msg = 'sent email:\n';
    if(bedrock.config.mail.log.headers) {
    ...
    if(bedrock.config.mail.log.text) {
      msg += '\n';
      msg += result.originalMessage.text;
    }
    if(bedrock.config.mail.log.html) {
      msg += '\n';
